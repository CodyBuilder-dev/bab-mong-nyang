...
const Feedreview = props => {
  ...
  useEffect(() => {
    dataFetch(store.url + "/review/" + props.f_No, "review");
  }, [store]);
                  ...
                  >
                    <Box display="flex" alignItems="center">
                      <ThumbUpAltIcon
                        ...
                        onClick={event => { 
                          axios({method:"POST",url : store.url + "/review/" , headers : store.headers , data : {r_No : data.r_No, u_No : store.u_No}}).then(res=>{
                            alert(res.data.message);
                            if(res.data.validation){
